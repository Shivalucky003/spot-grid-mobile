...
def build_grid(
    ...
    # Never allow a rounded grid level to become zero.
    # This can happen for extremely low-priced assets when
    # the calculated range is smaller than the exchange tick size.
    raw_levels = [
        price
        for price in raw_levels
        if price > 0
...
def evaluate_coin(
    ...
    for (
        ...
        # Calculate the actual first grid spacing safely.
        # Some very low-priced coins can round to 0 when
        # Binance tick-size precision is applied. Never divide
        # by zero and never allow a zero/negative grid level.
        if (
            len(grid) > 1
            and grid[0] > 0
            and grid[1] > grid[0]
