...
st.set_page_config(page_title="Binance Spot Grid Decision Support V7.1", layout="wide")
...
def get_session():
    s = requests.Session()
    s.headers.update({"User-Agent": "SpotGridDecisionSupport/7.1"})
    return s
...
def build_grid(lower, upper, n, mode, tick=0):
    ...
    n=max(1,int(n))
    if mode=="Geometric":
        levels=lower*(upper/lower)**(np.arange(n+1)/n)
    else:
        levels=np.linspace(lower,upper,n+1)
    if tick and tick>0:
        levels=np.unique([floor_step(x,tick) for x in levels])
...
def simulate(test, grid, capital, fee_pct, filters, inventory_pct, low_first):
    ...
    for _,row in test.iterrows():
        ...
        for a,bp in zip(path[:-1],path[1:]):
            if bp<grid[0] or bp>grid[-1]:
                escapes+=1
                outside=max(grid[0]-bp,bp-grid[-1],0)
                escape_severity += outside/(grid[-1]-grid[0])*100
    ...
    return {
        ...
        "max_dd":max_dd,"trades":trades,"buys":buys,"sells":sells,"cycles":cycles,
...
def score_row(r, feasible):
    s=50
    adx=r["ADX"]
    s += 18 if adx<20 else (8 if adx<25 else -12)
    s += 15 if r["Test Return %"]>5 else (7 if r["Test Return %"]>0 else -15)
    s += 10 if r["Test Alpha %"]>3 else (5 if r["Test Alpha %"]>0 else -8)
    dd=r["Max DD %"]
    s += 12 if dd<5 else (5 if dd<10 else (-8 if dd<15 else -15))
    s += 5 if r["Skipped %"]<=5 else (-8 if r["Skipped %"]>15 else 0)
    s += 5 if r["Escape Rate %"]<=5 else (-8 if r["Escape Rate %"]>15 else 0)
    s += 5 if r["Cycles"]>=10 else (-8 if r["Cycles"]<3 else 0)
    s += 7 if feasible else -20
    s -= 5 if r["Step %"] < r["Target Step %"]*0.75 else 0
    s=max(0,min(100,s))
    decision="🟢 CONSIDER" if s>=70 and r["Test Return %"]>0 and feasible else ("🟡 CAUTION" if s>=50 else "🔴 AVOID")
    return s,decision

def render_finder():
    st.title("⚡ Binance Spot Grid Decision Support V7.1")
    ...
    st.caption("The scanner no longer selects the highest-volume 5/10 coins. It first screens the entire eligible USDT universe, then scores markets using 5m, 1h and 4h behavior.")
    ...
    if st.button("🔎 Scan Binance + Multi-Timeframe Screen",type="primary"):
        ...
        # Fast first-pass liquidity screen. This is NOT the final ranking.
        valid=valid.sort_values("quoteVolume",ascending=False)
        # Analyze a broad pool so that the scanner does not need to request
        # multi-timeframe candles for every illiquid Binance pair.
        pool=valid.head(max(int(preselect_count)*4,80))
        ...
        for n,(_,trow) in enumerate(pool.iterrows()):
            ...
            if len(df5)<100 or len(df1)<120 or len(df4)<80:
                progress.progress((n+1)/len(pool))
                continue

            x5=df5.iloc[-1]; x1=df1.iloc[-1]; x4=df4.iloc[-1]

            # 5m: execution/noise quality
            adx5=float(x5["ADX"]) if np.isfinite(x5["ADX"]) else 99
            atr5=float(x5["ATR"]/x5["Close"]*100) if np.isfinite(x5["ATR"]) and x5["Close"]>0 else 99
            bb5=float(x5["BBWidthPct"]) if np.isfinite(x5["BBWidthPct"]) else 99

            # 1h: primary grid regime
            adx1=float(x1["ADX"]) if np.isfinite(x1["ADX"]) else 99
            atr1=float(x1["ATR"]/x1["Close"]*100) if np.isfinite(x1["ATR"]) and x1["Close"]>0 else 99
            bb1=float(x1["BBWidthPct"]) if np.isfinite(x1["BBWidthPct"]) else 99

            # 4h: larger trend / breakout risk
            adx4=float(x4["ADX"]) if np.isfinite(x4["ADX"]) else 99
            ...
            # Liquidity is a gate/quality factor, not the ranking itself.
        ...
        if not rows:
            st.error("No markets passed the multi-timeframe data-quality screen.")
            return

        pre_df=pd.DataFrame(rows).sort_values(
        ...
        st.success(
            f"Scanned {len(valid)} liquid USDT pairs; "
            f"multi-timeframe analysis completed for {len(pre_df)}; "
            f"selected {len(selected)} for the detailed backtest."
        )
        st.dataframe(pre_df.head(int(preselect_count)),use_container_width=True)
    ...
    if st.button("🚀 Run V7.1 Backtest"):
        ...
        for idx,symbol in enumerate(st.session_state.candidates):
            ...
            for name,candles,target in strategies:
                ...
                feasible=usdt_grid>=min_grid
                current=float(latest["Close"])
                position=max(0,min(100,(current-lower)/(upper-lower)*100))
                row={
    ...
    st.caption("Final ranking combines unseen test performance, alpha versus buy-and-hold, drawdown, completed cycles, skipped orders, range escapes, grid feasibility, and the multi-timeframe regime.")
    ...
    st.subheader("4. Risk interpretation")
    positives=[]; warnings=[]
    ...
    st.subheader("5. Manual Binance Grid Parameters")
    ...
    st.subheader("6. Proposed Grid Levels")
    ...
    st.subheader("8. Backtest Risk Metrics")
    st.dataframe(pd.DataFrame({
...
def render_active_audit():
    ...
    if st.button("🚀 Audit Active Running Grid", type="primary"):
        ...
        for _, row in recent.loc[below | above].iterrows():
            if row["Low"] < lower:
                severity_values.append((lower - float(row["Low"])) / lower * 100)
            if row["High"] > upper:
                severity_values.append((float(row["High"]) - upper) / upper * 100)
        ...
        st.dataframe(pd.DataFrame({
            "Parameter": [
                "Pair", "Lower", "Upper", "Grid Count", "Approx. Grid Step %",
                "Approx. USDT/Grid", "Trailing Up", "Stop Loss", "Take Profit",
                "Allocated Capital"
            ],
            "Value": [
                active_coin, lower, upper, int(active_grids), round(grid_step_pct, 3),
        ...
        levels = build_grid(lower, upper, int(active_grids), mode, tick)
        st.dataframe(
            pd.DataFrame({
...
def main():
    st.set_page_config(
        page_title="Binance Spot Grid Decision Support V8.2",
        layout="wide"
    )
    st.title("⚡ Binance Spot Grid Decision Support V8.2")
    st.info(
        f"**Data Freshness:** {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} UTC | "
