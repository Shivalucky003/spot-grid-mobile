...
def get_session():
    s = requests.Session()
    s.headers.update({"User-Agent": "SpotGridDecisionSupport/8.3"})
    return s
...
def render_finder():
    st.title("⚡ Binance Spot Grid Decision Support V8.3")
    ...
    if st.button("🚀 Run V8.3 Train/Test Backtest"):
...
def main():
    st.set_page_config(
        page_title="Binance Spot Grid Decision Support V8.3",
        layout="wide"
    )
    st.title("⚡ Binance Spot Grid Decision Support V8.3")
    st.info(
