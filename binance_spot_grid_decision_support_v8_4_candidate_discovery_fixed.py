...
def get_session():
    s = requests.Session()
    s.headers.update({"User-Agent": "SpotGridDecisionSupport/8.4"})
    return s
...
def render_finder():
    st.title("⚡ Binance Spot Grid Decision Support V8.4")
    ...
    if st.button("🔎 Scan Binance + Multi-Timeframe Screen",type="primary"):
        ...
        bands = np.array_split(valid, min(4, len(valid)))
        pool_parts = []
        per_band = max(1, math.ceil(target_scan / len(bands)))

        for band in bands:
            if band.empty:
                continue
            pool_parts.append(band.head(per_band))
    ...
    if st.button("🚀 Run V8.4 Train/Test Backtest"):
...
def main():
    st.set_page_config(
        page_title="Binance Spot Grid Decision Support V8.4",
        layout="wide"
    )
    st.title("⚡ Binance Spot Grid Decision Support V8.4")
    st.info(
