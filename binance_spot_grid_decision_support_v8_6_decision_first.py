...
def simulate(test, grid, capital, fee_pct, filters, inventory_pct, low_first):
    ...
    gross_profit=0.0; gross_loss=0.0; winning_cycles=0; losing_cycles=0; breakeven_cycles=0
        ...
        for a,bp in zip(path[:-1],path[1:]):
            if bp>a:
                ...
                for i in crossed:
                    ...
                    cycle_pnl=notional-fee-cost
                    realized+=cycle_pnl
                    if cycle_pnl > 1e-12:
                        gross_profit += cycle_pnl; winning_cycles += 1
                    elif cycle_pnl < -1e-12:
                        gross_loss += abs(cycle_pnl); losing_cycles += 1
                    else:
                        breakeven_cycles += 1
    ...
    profit_factor=(gross_profit/gross_loss) if gross_loss>1e-12 else (float("inf") if gross_profit>1e-12 else 0.0)
    win_rate=(winning_cycles/cycles*100) if cycles else 0.0
    avg_win=(gross_profit/winning_cycles) if winning_cycles else 0.0
    avg_loss=(gross_loss/losing_cycles) if losing_cycles else 0.0
    return {
        ...
        "max_dd":max_dd,"profit_factor":profit_factor,"win_rate":win_rate,
        "winning_cycles":winning_cycles,"losing_cycles":losing_cycles,"breakeven_cycles":breakeven_cycles,
        "gross_profit":gross_profit,"gross_loss":gross_loss,"avg_win":avg_win,"avg_loss":avg_loss,
        "trades":trades,"buys":buys,"sells":sells,"cycles":cycles,
...
def score_row(r, feasible):
    ...
    pf=float(r.get("Profit Factor",0) or 0) if str(r.get("Profit Factor",0)) != "∞" else float("inf")
    ...
    # Profit Factor is cycle-level, not total-profit divided by total-loss.
    if cycles >= 10:
        s += 8 if pf >= 2 else (5 if pf >= 1.5 else (2 if pf >= 1.2 else (-10 if pf < 1 else 0)))
    elif cycles >= 5:
        s += 3 if pf >= 1.5 else (-5 if pf < 1 else 0)
    else:
        s -= 3  # insufficient sample for a confident PF
...
def render_finder():
    st.title("⚡ Binance Spot Grid Decision Support V8.6.0")
    st.caption("Read-only decision support. You provide only your USDT balance, trading fee and minimum USDT per grid; the scanner determines the research settings automatically.")

    with st.sidebar:
        st.header("Your Inputs")
        capital=st.number_input("USDT Balance",min_value=10.0,value=160.0,step=10.0)
        fee_choice=st.selectbox("Trading Fee",["Standard 0.20%","BNB Discount 0.15%","Zero 0.00%"])
        fee_pct=0.20 if fee_choice.startswith("Standard") else (0.15 if fee_choice.startswith("BNB") else 0)
        min_grid=st.number_input("Minimum USDT per Grid",min_value=5.0,max_value=100.0,value=8.0,step=1.0)

    # Automatic research settings. These are deliberately hidden from the
    # normal controls: the user supplies capital/cost constraints; the
    # application decides how to screen and test the markets.
    mode="Geometric"
    preselect_count=25
    ...
    if st.button("🚀 Run V8.6 Train/Test Decision Backtest"):
        ...
        for idx,symbol in enumerate(st.session_state.candidates):
            ...
            for name,candles,target in strategies:
                ...
                row={
                    ...
                    "Profit Factor":c.get("profit_factor", 0.0),"Win Rate %":c.get("win_rate", 0.0),"Winning Cycles":c.get("winning_cycles", 0),"Losing Cycles":c.get("losing_cycles", 0),"Avg Win":c.get("avg_win", 0.0),"Avg Loss":c.get("avg_loss", 0.0),"Trades":c.get("trades", 0),"Cycles":c.get("cycles", 0),
    ...
    st.dataframe(pd.DataFrame({
        "Metric":["Test Return %","Buy-and-Hold %","Test Alpha %","Max Drawdown %","Profit Factor","Win Rate %","Winning Cycles","Losing Cycles","Average Win","Average Loss","Trades","Completed Cycles","Skipped Orders","Skipped %","Range Escape Bars","Range Escape Rate %","Escape Severity %","Intrabar Path Spread %"],
        "Value":[r["Test Return %"],r["Buy-Hold %"],r["Test Alpha %"],r["Max DD %"],r["Profit Factor"],r["Win Rate %"],r["Winning Cycles"],r["Losing Cycles"],r["Avg Win"],r["Avg Loss"],r["Trades"],r["Cycles"],r["Skipped"],r["Skipped %"],r["Escape Bars"],r["Escape Rate %"],r["Escape Severity %"],r["Path Spread %"]]
    }),use_container_width=True)
...
def _fmt_pf(x):
    if x is None or not np.isfinite(x):
        return "∞" if x is not None and x > 0 else "N/A"
    return f"{x:.2f}"


def _single_action_score(grid_result, hold_return):
    """Simple decision layer for a single coin. Higher is better."""
    if not grid_result:
        return -999, "🔴 AVOID", "No reliable grid backtest could be completed."
    g = grid_result
    alpha = g["total_return"] - hold_return
    pf = g["profit_factor"]
    cycles = g["cycles"]
    dd = g["max_dd"]
    esc = g["escape_rate"]
    score = 50.0
    score += max(-30, min(30, alpha * 6))
    score += 8 if pf >= 2 else (5 if pf >= 1.5 else (2 if pf >= 1.2 else (-10 if pf < 1 else 0)))
    score += 6 if cycles >= 20 else (3 if cycles >= 10 else (-5 if cycles < 5 else 0))
    score += 8 if dd < 5 else (3 if dd < 10 else (-8 if dd < 15 else -15))
    score += 5 if esc <= 5 else (-5 if esc > 15 else 0)
    score = max(0, min(100, score))

    if alpha >= 2 and cycles >= 5 and pf >= 1.2 and dd < 10 and esc <= 15:
        return score, "🟢 GRID IS WORTH CONSIDERING", "The tested grid has a positive advantage over buy-and-hold with acceptable execution evidence."
    if alpha <= -2:
        return score, "🔴 BUY & HOLD / WAIT", "Buy-and-hold performed better in the unseen test period; the grid has a negative opportunity cost."
    if cycles < 5:
        return score, "🟡 WAIT / INSUFFICIENT EVIDENCE", "Too few completed grid cycles to trust the grid result."
    if pf < 1.0:
        return score, "🔴 AVOID GRID", "Completed grid cycles lost more than they won on a gross P&L basis."
    return score, "🟡 REVIEW", "Neither approach has a strong enough advantage to make the choice automatic."


def render_single_coin():
    st.header("🧠 Single-Coin Decision")
    st.caption("Enter a coin you are considering. The analyzer compares grid, buy-and-hold and waiting conditions instead of simply ranking the coin.")

    c1, c2 = st.columns(2)
    with c1:
        symbol = st.text_input("Coin", value="AVAX", key="single_coin").upper().strip()
        usdt = st.number_input("USDT Available", min_value=0.0, value=160.0, step=10.0, key="single_usdt")
        coin_qty = st.number_input("Coin Inventory", min_value=0.0, value=0.0, step=0.1, key="single_qty")
    with c2:
        avg_cost = st.number_input("Average Coin Cost (0 = unknown)", min_value=0.0, value=0.0, step=0.01, key="single_avg")
        fee_choice = st.selectbox("Trading Fee", ["Standard 0.20%", "BNB Discount 0.15%", "Zero 0.00%"], key="single_fee")
        fee_pct = 0.20 if fee_choice.startswith("Standard") else (0.15 if fee_choice.startswith("BNB") else 0.0)
        min_grid = st.number_input("Minimum USDT per Grid", min_value=5.0, max_value=100.0, value=8.0, step=1.0, key="single_min_grid")

    if st.button("🔎 Analyze This Coin", type="primary"):
        symbol = symbol.replace("/", "").replace("-", "")
        if not symbol:
            st.error("Enter a coin symbol, for example AVAX or AVAXUSDT.")
            return
        ...
        if symbol not in symbols:
            st.error(f"{symbol} is not available as a Binance Spot USDT market.")
            return

        with st.spinner(f"Analyzing {symbol} across 5m / 1h / 4h and testing grid alternatives..."):
            df5 = fetch_klines(symbol, "5m", 288)
            df1 = fetch_klines(symbol, "1h", 336)
            df4 = fetch_klines(symbol, "4h", 168)
            ticker = fetch_24h()
        ...
        if len(df1) < 120:
            st.error("Not enough 1h history to make a reliable decision.")
            return

        current = float(df1["Close"].iloc[-1])
        coin_value = coin_qty * current
        portfolio_value = usdt + coin_value
        if portfolio_value <= 0:
            st.error("Enter some USDT or coin inventory before analyzing.")
            return

        inventory_pct_now = coin_value / portfolio_value

        # Current multi-timeframe condition.
        def last_metric(df, col):
            if df.empty or col not in df:
                return np.nan
            v = df[col].iloc[-1]
            return float(v) if pd.notna(v) else np.nan

        adx5, rsi5 = last_metric(df5, "ADX"), last_metric(df5, "RSI")
        adx1, rsi1 = last_metric(df1, "ADX"), last_metric(df1, "RSI")
        atr1 = last_metric(df1, "ATR")
        adx4, rsi4 = last_metric(df4, "ADX"), last_metric(df4, "RSI")
        ema50_4 = last_metric(df4, "EMA50")
        ema200_4 = last_metric(df4, "EMA200")

        trend_warning = []
        if np.isfinite(adx4) and adx4 >= 30:
            trend_warning.append("4h market is strongly directional; this can make a spot grid underperform holding the coin.")
        if np.isfinite(adx1) and adx1 >= 30:
            trend_warning.append("1h market is strongly directional; grid range escapes are more likely.")
        if np.isfinite(ema50_4) and np.isfinite(ema200_4):
            if current > ema50_4 > ema200_4:
                trend_warning.append("4h structure is bullish; repeatedly selling grid inventory can sacrifice upside exposure.")
            elif current < ema50_4 < ema200_4:
                trend_warning.append("4h structure is bearish; a spot grid can accumulate a falling asset.")

        # Test the 3 grid horizons. Settings are automatic.
        strategies = [("3D", 72, 0.8), ("7D", 168, 1.2), ("14D", 336, 1.8)]
        ...
        for name, candles, target in strategies:
            if len(df1) < candles:
                continue
            sub = df1.tail(candles).copy()
            split = int(len(sub) * 0.70)
            train, test = sub.iloc[:split], sub.iloc[split:]
            if len(train) < 30 or len(test) < 15:
                continue
            rng = make_range(train, 1.5)
            if not rng:
                continue
            lower, upper = rng
            width_pct = (upper - lower) / lower * 100 if lower > 0 else 0
            n = max(5, min(100, int(width_pct / max(target, 0.1))))
            n = min(n, max(5, int(total_capital / max(min_grid, 1))))
            bt = backtest(test, lower, upper, n, total_capital, fee_pct, "Geometric", filt, inventory_pct_now)
            if not bt:
                continue
            g = bt["conservative"]
            # Portfolio-level buy-and-hold benchmark with the user's actual mix.
            p0 = float(test["Close"].iloc[0]); p1 = float(test["Close"].iloc[-1])
            if p0 <= 0: continue
            hold_final = usdt + coin_qty * p1
            if total_capital > 0:
                hold_return = (hold_final / total_capital - 1) * 100
            else:
                hold_return = (p1 / p0 - 1) * 100
            # The grid simulation is normalized to the same starting portfolio value.
            alpha = g["total_return"] - hold_return
            score, action, reason = _single_action_score(g, hold_return)
            rows.append({
                "Strategy": name, "Action": action, "Score": round(score, 0),
                "Grid Return %": round(g["total_return"], 2), "Hold Return %": round(hold_return, 2),
                "Advantage %": round(alpha, 2), "Profit Factor": _fmt_pf(g["profit_factor"]),
                "Win Rate %": round(g["win_rate"], 1), "Cycles": g["cycles"],
                "Max DD %": round(g["max_dd"], 2), "Escape Rate %": round(g["escape_rate"], 2),
                "Escape Severity %": round(g["escape_severity"], 2), "Skipped %": round(g["skip_rate"], 2),
                "Lower": lower, "Upper": upper, "Grid Levels": len(bt["grid"]),
                "Step %": round(((bt["grid"][1]-bt["grid"][0])/bt["grid"][0]*100) if len(bt["grid"])>1 and bt["grid"][0]>0 else 0, 2),
                "Capital": round(total_capital, 2), "Reason": reason
            })

        if not rows:
            st.error("The coin did not produce a usable grid test. This is not a recommendation to deploy.")
            return

        result_df = pd.DataFrame(rows).sort_values(["Score", "Advantage %"], ascending=[False, False]).reset_index(drop=True)
        best = result_df.iloc[0]

        st.subheader(f"🏆 What should you do with {symbol}?")
        if str(best["Action"]).startswith("🟢"):
            st.success(f"{best['Action']} — Best setup: {best['Strategy']} | {best['Reason']}")
        elif str(best["Action"]).startswith("🔴"):
            st.error(f"{best['Action']} — {best['Reason']}")
        else:
            st.warning(f"{best['Action']} — {best['Reason']}")

        m1,m2,m3,m4 = st.columns(4)
        m1.metric("Best Strategy", best["Strategy"])
        m2.metric("Grid vs Hold", f"{best['Advantage %']:+.2f}%")
        m3.metric("Profit Factor", best["Profit Factor"])
        m4.metric("Max Drawdown", f"{best['Max DD %']:.2f}%")

        st.subheader("💰 Your Starting Position")
        inv_rows = [
            ["USDT", f"{usdt:,.2f}"], [f"{symbol} inventory", f"{coin_qty:,.8f}"],
            ["Current coin value", f"{coin_value:,.2f} USDT"], ["Total starting value", f"{portfolio_value:,.2f} USDT"],
            ["Inventory share", f"{inventory_pct_now*100:.1f}%"]
        ]
        if avg_cost > 0 and coin_qty > 0:
            inv_rows.append(["Current unrealized P&L", f"{(current-avg_cost)*coin_qty:+,.2f} USDT"])
        st.dataframe(pd.DataFrame(inv_rows, columns=["Item", "Value"]), use_container_width=True, hide_index=True)

        st.subheader("⚖️ Grid vs Buy & Hold")
        st.dataframe(result_df[["Strategy","Action","Grid Return %","Hold Return %","Advantage %","Profit Factor","Win Rate %","Cycles","Max DD %","Escape Rate %"]], use_container_width=True, hide_index=True)

        st.subheader("📍 Best Grid Setup")
        st.dataframe(pd.DataFrame({
            "Parameter":["Coin","Strategy","Lower","Upper","Grid Levels","Step %","Capital Basis"],
            "Value":[symbol,best["Strategy"],best["Lower"],best["Upper"],best["Grid Levels"],best["Step %"],best["Capital"]]
        }), use_container_width=True, hide_index=True)

        st.subheader("⏱️ Current Multi-Timeframe Condition")
        mt = pd.DataFrame([
            ["5m", adx5, rsi5, "Execution / short-term"],
            ["1h", adx1, rsi1, "Main grid regime"],
            ["4h", adx4, rsi4, "Trend / opportunity-cost risk"]
        ], columns=["Timeframe","ADX","RSI","What it means"])
        st.dataframe(mt, use_container_width=True, hide_index=True)

        for w in trend_warning:
            st.warning("⚠️ " + w)

        if avg_cost > 0 and coin_qty > 0:
            st.info("Your average purchase price is used only to show your existing position's unrealized P&L. The forward grid-vs-hold decision is based on current portfolio value, so an old cost basis does not artificially make one choice look better.")

        with st.expander("🔬 Technical details — calculations the bot used"):
            st.write("The normal controls are intentionally kept simple. The analyzer automatically used 5m, 1h and 4h data, 70/30 train-test separation, ATR-based range construction, multi-path conservative backtesting, fees, grid feasibility, Profit Factor, drawdown and escape metrics.")
            if not ticker.empty:
                tr = ticker[ticker["symbol"]==symbol]
                if not tr.empty:
                    st.write(f"24h quote volume: {float(tr.iloc[0]['quoteVolume']):,.0f} USDT | 24h change: {float(tr.iloc[0]['priceChangePercent']):+.2f}%")

def main():
    st.set_page_config(
        page_title="Binance Spot Grid Decision Support V8.6.0",
        layout="wide"
    )
    st.title("⚡ Binance Spot Grid Decision Support V8.6.0")
    st.info(
