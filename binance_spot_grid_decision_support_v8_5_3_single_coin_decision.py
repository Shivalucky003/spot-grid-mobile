...
def get_session():
    s = requests.Session()
    s.headers.update({"User-Agent": "SpotGridDecisionSupport/8.5.2"})
    return s
...
def render_finder():
    st.title("⚡ Binance Spot Grid Decision Support V8.5.2")
...
def render_coin_analyzer():
    """Third tab: user enters one coin and receives a plain-language decision.

    This is intentionally decision support rather than an order-entry screen.
    It combines 1m/5m execution conditions with 1h/4h regime information and
    tests three grid horizons using the same backtest engine as the Finder.
    """
    st.header("🧠 Single-Coin Decision Analyzer")
    st.caption(
        "Enter one Binance USDT spot coin. The analyzer calculates the indicators, "
        "tests grid candidates and tells you what the evidence supports: use a grid, "
        "wait/recheck, or avoid a grid. It does not place orders."
    )

    c1, c2, c3 = st.columns([1.3, 1, 1])
    with c1:
        symbol_input = st.text_input("Coin / Symbol", value="", placeholder="Example: AVAX or AVAXUSDT")
    with c2:
        capital = st.number_input("Capital (USDT)", min_value=10.0, value=160.0, step=10.0, key="an_capital")
    with c3:
        fee_choice = st.selectbox(
            "Round-trip fee",
            ["Standard 0.20%", "BNB Discount 0.15%", "Zero 0.00%"],
            key="an_fee"
        )

    c4, c5, c6 = st.columns(3)
    with c4:
        mode = st.selectbox("Grid Mode", ["Arithmetic", "Geometric"], key="an_mode")
    with c5:
        atr_mult = st.slider("ATR Range Buffer", 0.5, 3.0, 1.5, 0.1, key="an_atr")
    with c6:
        inventory_pct = st.slider("Starting Coin Inventory %", 0, 50, 0, 5, key="an_inv")

    analyze = st.button("🔬 Analyze This Coin", type="primary", key="analyze_coin")
    if not analyze:
        st.info(
            "Enter a symbol such as AVAX or AVAXUSDT and press Analyze. "
            "The tool will calculate 1m, 5m, 1h and 4h conditions before making the recommendation."
        )
        return

    symbol = symbol_input.strip().upper().replace("/", "")
    if not symbol:
        st.warning("Enter a coin symbol first.")
        return
    ...
    if symbol not in symbols:
        st.error(f"{symbol} is not currently available as a Binance USDT spot trading pair in this scanner.")
        return

    fee_pct = 0.20 if fee_choice.startswith("Standard") else (0.15 if fee_choice.startswith("BNB") else 0.0)
    exchange_filter = filters.get(symbol, {})

    with st.spinner(f"Analyzing {symbol}: 1m / 5m / 1h / 4h data, grid ranges and backtests..."):
        df1m = fetch_klines(symbol, "1m", 1000)
        df5 = fetch_klines(symbol, "5m", 1000)
        df1h = fetch_klines(symbol, "1h", 1000)
        df4h = fetch_klines(symbol, "4h", 1000)
        ticker = fetch_24h()

    if df1h.empty or df4h.empty:
        st.error("Not enough 1h/4h data was returned for this coin. Try again later.")
        return

    latest1m = df1m.iloc[-1] if not df1m.empty else None
    latest5 = df5.iloc[-1] if not df5.empty else None
    latest1h = df1h.iloc[-1]
    latest4h = df4h.iloc[-1]
    price = float(latest1h["Close"])

    # ---------- Current market state ----------
    st.subheader(f"📌 {symbol} — Current Market State")
    cards = st.columns(6)
    cards[0].metric("Price", f"{price:.8g}")
    cards[1].metric("1h Regime", str(latest1h["Regime"]))
    cards[2].metric("1h ADX", f"{float(latest1h['ADX']):.1f}")
    cards[3].metric("1h RSI", f"{float(latest1h['RSI']):.1f}")
    cards[4].metric("1h ATR %", f"{float(latest1h['ATR']/price*100):.2f}%")
    cards[5].metric("4h ADX", f"{float(latest4h['ADX']):.1f}")

    tf_rows = []
    for label, df in [("1m", df1m), ("5m", df5), ("1h", df1h), ("4h", df4h)]:
        if df.empty:
            continue
        x = df.iloc[-1]
        close = float(x["Close"])
        atr_pct = float(x["ATR"] / close * 100) if close > 0 and np.isfinite(x["ATR"]) else np.nan
        tf_rows.append({
            "Timeframe": label,
            "Regime": str(x["Regime"]),
            "ADX": round(float(x["ADX"]), 1),
            "RSI": round(float(x["RSI"]), 1),
            "EMA9": round(float(x["EMA9"]), 8) if np.isfinite(x["EMA9"]) else np.nan,
            "EMA21": round(float(x["EMA21"]), 8) if np.isfinite(x["EMA21"]) else np.nan,
            "MACD Hist": round(float(x["MACDHist"]), 8) if np.isfinite(x["MACDHist"]) else np.nan,
            "ATR %": round(atr_pct, 3) if np.isfinite(atr_pct) else np.nan,
            "BB Width %": round(float(x["BBWidthPct"]), 2) if np.isfinite(x["BBWidthPct"]) else np.nan,
        })
    st.dataframe(pd.DataFrame(tf_rows), use_container_width=True, hide_index=True)

    # ---------- Build and test three grid horizons ----------
    horizons = [
        ("Tight 3D", 72, 0.8),
        ("Moderate 7D", 168, 1.2),
        ("Wide 14D", 336, 1.8),
    ...
    for name, bars, target_step in horizons:
        if len(df1h) < bars + 50:
            continue
        window = df1h.tail(bars).copy()
        rng = make_range(window, atr_mult)
        if not rng:
            continue
        lower, upper = rng
        width_pct = (upper - lower) / lower * 100
        n = max(5, min(150, int(width_pct / target_step)))
        bt = backtest(window, lower, upper, n, capital, fee_pct, mode, exchange_filter, inventory_pct / 100)
        if not bt:
            continue
        c = bt["conservative"]
        step_pct = (bt["grid"][1] - bt["grid"][0]) / bt["grid"][0] * 100 if len(bt["grid"]) > 1 else 0
        feasible = (n * max(5.0, capital / max(n, 1))) <= capital + 1e-9
        row = {
            "Strategy": name,
            "Lower": lower,
            "Upper": upper,
            "Range Width %": width_pct,
            "Grid Levels": len(bt["grid"]),
            "Step %": step_pct,
            "Test Return %": c["total_return"],
            "Buy & Hold %": c["buy_hold"],
            "Grid Advantage %": c["alpha"],
            "Profit Factor": c["profit_factor"],
            "Win Rate %": c["win_rate"],
            "Max DD %": c["max_dd"],
            "Cycles": c["cycles"],
            "Escape Rate %": c["escape_rate"],
            "Escape Severity %": c["escape_severity"],
            "Skipped %": c["skip_rate"],
        }
        # Same core score logic used by Finder.
        score, decision = score_row(row, feasible)
        row["Score"] = score
        row["Decision"] = decision
        analysis_rows.append(row)
        detailed[name] = (window, bt, c)

    if not analysis_rows:
        st.error("The coin could not produce a valid grid backtest with the current settings.")
        return

    results = pd.DataFrame(analysis_rows).sort_values(
        ["Score", "Grid Advantage %"], ascending=[False, False]
    ).reset_index(drop=True)
    best = results.iloc[0]

    # ---------- Plain-language decision ----------
    st.subheader("🧭 What Should You Do With This Coin?")
    decision = str(best["Decision"])
    pf = best["Profit Factor"]
    pf_text = "∞" if isinstance(pf, (float, np.floating)) and np.isinf(pf) else f"{float(pf):.2f}"

    if decision.startswith("🟢"):
        headline = "GRID IS WORTH CONSIDERING"
        st.success(
            f"{headline}: the {best['Strategy']} setup has the strongest evidence among the tested ranges. "
            f"Score {best['Score']:.0f}/100, grid advantage {best['Grid Advantage %']:+.2f}%, "
            f"Profit Factor {pf_text}, max drawdown {best['Max DD %']:.2f}%."
        )
    elif decision.startswith("🔴"):
        headline = "DO NOT DEPLOY A GRID YET"
        st.error(
            f"{headline}: the best tested setup does not currently provide strong enough evidence. "
            f"Best score {best['Score']:.0f}/100, grid advantage {best['Grid Advantage %']:+.2f}%, "
            f"Profit Factor {pf_text}, escape rate {best['Escape Rate %']:.2f}%."
        )
    else:
        headline = "WAIT / REVIEW"
        st.warning(
            f"{headline}: there may be a usable range, but the evidence is not strong enough for a confident grid decision. "
            f"Best score {best['Score']:.0f}/100 and {int(best['Cycles'])} completed cycles."
        )

    m = st.columns(6)
    m[0].metric("Recommended", str(best["Strategy"]))
    m[1].metric("Lower", f"{best['Lower']:.8g}")
    m[2].metric("Upper", f"{best['Upper']:.8g}")
    m[3].metric("Grid Levels", str(int(best["Grid Levels"])))
    m[4].metric("Step", f"{best['Step %']:.2f}%")
    m[5].metric("Score", f"{best['Score']:.0f}/100")

    st.subheader("📊 Tested Grid Options")
    display = results.copy()
    for col in ["Lower", "Upper"]:
        display[col] = display[col].map(lambda x: f"{x:.8g}")
    for col in ["Range Width %", "Step %", "Test Return %", "Buy & Hold %", "Grid Advantage %", "Win Rate %", "Max DD %", "Escape Rate %", "Escape Severity %", "Skipped %"]:
        display[col] = display[col].map(lambda x: f"{x:.2f}%")
    display["Profit Factor"] = display["Profit Factor"].map(
        lambda x: "∞" if np.isinf(float(x)) else f"{float(x):.2f}"
    )
    st.dataframe(
        display[["Strategy","Decision","Score","Lower","Upper","Grid Levels","Step %",
                  "Test Return %","Buy & Hold %","Grid Advantage %","Profit Factor","Win Rate %",
                  "Max DD %","Cycles","Escape Rate %","Escape Severity %","Skipped %"]],
        use_container_width=True, hide_index=True
    )

    # ---------- Why ----------
    st.subheader("🔎 Why the Analyzer Reached This Decision")
    reasons = []
    if float(latest1h["ADX"]) < 20:
        reasons.append("1h ADX is below 20, which supports a ranging/grid environment.")
    elif float(latest1h["ADX"]) >= 25:
        reasons.append("1h ADX is at/above 25, indicating stronger trend conditions that are less suitable for a normal range grid.")
    else:
        reasons.append("1h ADX is in the transition zone; the market is not clearly ranging.")

    if float(latest4h["ADX"]) >= 25:
        reasons.append("4h ADX is elevated, so the higher-timeframe trend is a risk to a range grid.")
    else:
        reasons.append("4h ADX is below 25, so the higher timeframe is not strongly trending.")

    if latest5 is not None:
        if float(latest5["ADX"]) >= 30:
            reasons.append("5m ADX is elevated; short-term movement is currently aggressive, so execution conditions may be unstable.")
        elif float(latest5["ADX"]) < 20:
            reasons.append("5m ADX is low, which is supportive of calmer short-term grid execution.")

    if float(best["Escape Rate %"]) > 15:
        reasons.append("The selected range is escaping frequently; this is a major warning.")
    elif float(best["Escape Rate %"]) > 5:
        reasons.append("The selected range has a moderate escape rate; monitor the range closely.")
    else:
        reasons.append("The selected range has a relatively low historical escape rate in the test period.")

    if int(best["Cycles"]) < 5:
        reasons.append("The test produced fewer than five completed cycles, so Profit Factor is not yet strong evidence.")
    elif float(best["Profit Factor"]) >= 1.5 or (np.isinf(float(best["Profit Factor"]))):
        reasons.append("Cycle-level Profit Factor is supportive, with enough completed cycles to make it useful evidence.")
    else:
        reasons.append("Cycle-level Profit Factor is weak; this reduces confidence in the grid.")

    for reason in reasons:
        st.write("• " + reason)

    st.info(
        "The analyzer does not assume that every coin should use a grid. Its job is to test the evidence "
        "and tell you when a grid is not the appropriate choice. The 1m/5m data is mainly used for execution "
        "conditions; the 1h/4h data drives the range/regime decision."
    )

    with st.expander("🧮 Show all calculations for the selected setup"):
        window, bt, c = detailed[str(best["Strategy"])]
        st.write("**Grid levels:**")
        st.dataframe(
            pd.DataFrame({"Level": range(1, len(bt["grid"])+1), "Price": bt["grid"]}),
            use_container_width=True, hide_index=True
        )
        st.write({
            "Final equity": round(c["final_equity"], 4),
            "Realized P&L": round(c["realized"], 4),
            "Gross profit": round(c["gross_profit"], 4),
            "Gross loss": round(c["gross_loss"], 4),
            "Average win": round(c["avg_win"], 4),
            "Average loss": round(c["avg_loss"], 4),
            "Winning cycles": c["winning_cycles"],
            "Losing cycles": c["losing_cycles"],
            "Breakeven cycles": c["breakeven_cycles"],
            "Skipped orders": c["skipped"],
            "Escape rate %": round(c["escape_rate"], 3),
            "Escape severity %": round(c["escape_severity"], 3),
            "Path spread %": round(bt["path_spread"], 3),
        })

def main():
    st.set_page_config(
        page_title="Binance Spot Grid Decision Support V8.5.2",
        layout="wide"
    )
    st.title("⚡ Binance Spot Grid Decision Support V8.5.2")
    st.info(
